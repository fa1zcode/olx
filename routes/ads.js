...
module.exports = function (db) {
  ...
  router.post("/edit/:id", helpers.isLoggedIn, function (req, res) {
    const id = Number(req.params.id);
    let fileNames = req.body.pictures
      ? typeof req.body.pictures == "string"
        ? [req.body.pictures]
        : [...req.body.pictures]
      : [];
    if (!req.files || Object.keys(req.files).length === 0) {
      db.query(
        "update ads set title = $1, description = $2, category = $3, seller = $4, price = $5, approved = $6, pictures = $7 where id = $8",
        [
          req.body.title,
          req.body.description,
          Number(req.body.category),
  ...
  router.post("/data/:id", function (req, res) {
    ...
    db.query(
      ...
      [req.body.title, req.body.description, Number(req.body.price), id],
      (err, data) => {
        if (err) return res.json({ err: err });
        res.json(data.rows);
      }
