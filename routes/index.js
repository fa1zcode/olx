...
module.exports = function (db) {
  ...
  router.post("/sell", helpers.isLoggedIn, function (req, res) {
    if (!req.files || Object.keys(req.files).length === 0) {
      db.query(
        "insert into ads(title, description, category, seller, price, approved, pictures) values ($1, $2, $3, $4, $5, $6, $7)",
        [
          req.body.title,
  ...
  router.get("/profile", helpers.isLoggedIn, function (req, res) {
    db.query(
      ...
      (err, item) => {
        if (err) return res.send(err);
        db.query(
          "select * from ads where seller = $1",
          [req.session.user.id],
          (err, ads) => {
            if (err) return res.send(err);
            res.render("profile", {
  ...
  router.post("/profile", helpers.isLoggedIn, function (req, res) {
    ...
    if (!req.files || Object.keys(req.files).length === 0) {
      db.query(
        ...
        [req.body.fullname, req.body.email, req.body.phone, id],
        (err) => {
          if (err) {
            console.log(err);
            req.flash("successMessage", `gagal mengubah profile`);
      ...
      file.mv(uploadPath, function (err) {
        if (err) return res.status(500).send(err);
        db.query(
          "update users set fullname = $1, email = $2, phone = $3, avatar = $4 where id = $5",
          [req.body.fullname, req.body.email, req.body.phone, fileName, id],
          (err) => {
            if (err) {
              console.log(err);
              req.flash("successMessage", `gagal mengubah profile dan upload avatar`);
              return res.redirect("/profile");
            }
            res.redirect("/profile");
          }
